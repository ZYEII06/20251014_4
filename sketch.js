...
function spawnExplosion() {
  ...
  if (popSound.isLoaded()) { 
    // popSound.rate(random(0.9, 1.1)); // 可選：加入隨機播放速度讓音效更生動
    popSound.play();
  }
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    if (useFallback) {
      ...
      chosenColor = random() < 0.5 ? macaronColors[baseIdx] : macaronColors[secondIdx];
    }
    ex.particles.push({
      angle: angle,
  ...
  for (let i = 0; i < respawnCount; i++) {
    ...
    circles[idx].x = ex.x + random(-ex.maxRadius, ex.maxRadius);
